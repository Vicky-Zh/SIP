...
def SIP_NetMF_embedding(A, A1, embedding_origin, evals1, s1, sum_eval, vol, deepwalk_matrix, u1, v1, evecs1, dim=32):
    """
    ...
    embedding_all = np.vstack((embedding_origin, simu_embed_all))
...
def pred_over_time(t_begin, t_end, t_gap, C = 1.):
    ...
    for t in range(t_begin, t_end, 1):
        ...
        for tG in range(future_num):
            ...
            vol = float(A.sum())
            embedding_all, time_embed = SIP_NetMF_embedding(A, A1, embedding_origin, evals1, s1, sum_eval, vol, deepwalk_matrix, u1, v1, evecs1, dim=32)
...
if __name__ == "__main__":
    ...
    for t in t_gap:
        ...
        data.to_csv('data/'+str(name)+'/tGap_SIP-NetMF/accuarcy_tGap='+str(int(t))+'.csv')
        t_num = t_num + 1