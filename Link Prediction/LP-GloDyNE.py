...
name = 'LP-superuser'
file = 'dynamicgem/embedding/sx-superuser-washed.txt'
...
def predict_cv(X, y, train_ratio=0.9, n_splits=5, random_state=0, clf= LogisticRegression()):#这是最主要的一个函数
    F1_score, AUC_score = [], []
...
def pred_over_time(t_begin, t_end, t_gap, C = 1.):
    ...
    for t in range(t_begin, t_end, 1):
        ...
        if (t-1 in l) and (t in l):
        #if True:
            G=nx.Graph() 
            node_num = np.max(CM_origin[0:l.index(t),0:2])
            for i in range(node_num+1):
                G.add_node(i)

            edge_num = l.index(t-1)
            ...
            edge_num_new = l.index(t)
            edges = CM_origin[edge_num:edge_num_new,0:2].tolist()
            G.add_edges_from(edges)

            graphs.append(G.copy())

            from libne import DynWalks  
            model = DynWalks.DynWalks(G_dynamic=graphs, limit=0.1, emb_dim=32, scheme=4,
                                        num_walks=10, walk_length=80, window=10, negative=5,
                                        workers=32, seed=42)
            model.sampling_traning()
            dics = model.emb_dicts[0]
            embedding = np.array([dics[key] for key in dics])        
            np.save('data/'+str(name)+'/tGap_GloDyNE/Embedding/' + str(t) + '_train.npy', embedding)

            X,y = create_embedding_sample(embedding, t)
            print('The number of edges at time t ='+str(X.shape[0]))

            F1, AUC, classifier = predict_cv(X, y, train_ratio=0.9, n_splits=5, random_state=0, clf = classifier)
            print('F1='+ str(F1))
            print('AUC='+ str(AUC))
            F1_all.append(F1)
            AUC_all.append(AUC)
...
#####################################
            for tG in range(future_num):
                print('m = ' +str(t_gap[tG]))
                m = t_gap[tG]
                k = 0
                if (t+m) in l:
                    graphs_new = []

                    node_num_m = np.max(CM_origin[0:l.index(t+m),0:2])
                    print('node_num_m='+str(node_num_m))

                    for G_before in graphs:
                        for node in range(node_num+1, node_num_m+1,1):
                            G_before.add_node(node)
                        graphs_new.append(G_before)
                
                    for t_prev in range(int(t+1),int(t+m+1),1):
                        t_prev=int(t_prev)
                        if t_prev in l:
                            G=nx.Graph()
                            for i in range(node_num_m+1):
                                G.add_node(i)
                            edge_end = l.index(t_prev)
                            edges = CM_origin[0:edge_end,0:2].tolist()
                            
                            G.add_edges_from(edges)

                            graphs_new.append(G.copy())
                            k = k+1

                    t1 = time.time()
                    model = DynWalks.DynWalks(G_dynamic=graphs_new, limit=0.1, emb_dim=32, scheme=4,
                                        num_walks=10, walk_length=80, window=10, negative=5,
                                        workers=32, seed=42)
                    model.sampling_traning()
                    dics = model.emb_dicts[0]
                    embedding_all = np.array([dics[key] for key in dics])        
                    t2 = time.time()
                    time_total = t2-t1
                    np.save('data/'+str(name)+'/tGap_GloDyNE/Embedding/' + str(t) + '_m=' + str(m) +'_test.npy', embedding_all)

                    X_future, y_future = create_embedding_sample(embedding_all, t+m)
                    print('The number of edges at time t ='+str(X_future.shape[0]))
                    ...
                    print('F1='+ str(f1_future))
                    print('AUC='+ str(auc_future))
...
if __name__ == "__main__":
    ...
    for t in t_gap:
        ...
        data.to_csv('data/'+str(name)+'/tGap_GloDyNE/accuarcy_netmf_tGap='+str(int(t))+'.csv')
        t_num = t_num + 1
