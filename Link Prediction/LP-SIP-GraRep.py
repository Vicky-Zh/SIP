...
def create_target_matrix(A_tilde, A_hat):
    ...
    return A_tilde, sparse.lil_matrix(target_matrix)

def SIP_GraRep_embedding(A, A1,embedding, basis, dimensions = 8, order= 4, dim = 32):
    n = A1.shape[0]
    n1 = A.shape[0]
    _, D = csgraph.laplacian(A, normed=False, return_diag=True)
    M_origin = sparse.diags(D ** -1).dot(A)
    M = copy.deepcopy(M_origin)
    simu_embed = sparse.lil_matrix(((n1-n),dim))
...
def pred_over_time(t_begin, t_end, t_gap, C = 1.):
    ...
    for t in range(t_begin, t_end, 1):
        ...
        model.fit(G1)
        embedding_origin = model.get_embedding()
        basis = model._basis
        ...
        for tG in range(future_num):
            ...
            embedding_all, time_embed = SIP_GraRep_embedding(A, A1,embedding_origin, basis, dim = 32)
...
if __name__ == "__main__":
    ...
    for t in t_gap:
        ...
        data.to_csv('data/'+str(name)+'/tGap_SIP-AROPE/accuarcy_tGap='+str(int(t))+'.csv')
        t_num = t_num + 1