...
def pred_over_time(t_begin, t_end, t_gap, C = 1.):
    ...
    for t in range(t_begin, t_end, 1):
        ...
        if (t in l) and ((t+1) in l):
            ...
            model = NodeSketch(dimensions=32)
            model.fit(G)
    ...
    #####################################
            for tG in range(future_num):
                if ((t+t_gap[tG]) in l) and ((t+t_gap[tG]+1) in l):
...
if __name__ == "__main__":
    ...
    for t in t_gap:
        ...
        data.to_csv('data/'+ str(name) +'/tGap_NodeSketch/accuarcy_netmf_tGap='+str(int(t))+'.csv')
        t_num = t_num + 1