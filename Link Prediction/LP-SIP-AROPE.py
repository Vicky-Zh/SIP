...
def AROPE(A, dim, order = [3], weights = [[1, 0.01, 0.0001]]):
...
def SIP_AROPE_embedding(A, A1, embedding, basis, dim = 32):
...
def pred_over_time(t_begin, t_end, t_gap, C = 1.):
    ...
    for t in range(t_begin, t_end, 1):
        ...
        embedding_origin, basis = AROPE(A1, dim)
        print(embedding_origin.shape)
        ...
        for tG in range(future_num):
            ...
            embedding_all, time_embed = SIP_AROPE_embedding(A, A1, embedding_origin, basis, dim = 32)
